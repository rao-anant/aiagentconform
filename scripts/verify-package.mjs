...
const install = mkdtempSync(path.join(tmpdir(), 'agentcheck-package-'));
try {
  ...
  const executable = path.join(install, 'node_modules', '.bin', 'agentcheck');
  ...
  assert.equal(execFileSync(npx, ['--no-install', 'agentcheck', '--version'], { ...npmOptions, cwd: install }).trim(), pkg.version);
  assert.match(run(['--help'], 0), /--list-rules/);
  assert.equal(run([fixture('valid')], 0), 'AgentCheck - Agent Plugins 1.0\n✓ 20/20 conformance checks passed\n');
